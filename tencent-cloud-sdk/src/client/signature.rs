...
use reqwest::{
    Request, Response,
    header::{self, AUTHORIZATION, HeaderMap, HeaderName},
};
...
pub fn add_authorization(req: &mut Request, context: &SignatureContext) -> anyhow::Result<String> {
    ...
    req.headers_mut().insert(TC_VERSION, context.version.parse().unwrap());
    ...
    let derived_sk = hmac_sha256(
        hmac_sha256(
            hmac_sha256(format!("TC3{}", context.sk).as_bytes(), format!("{date}").as_bytes()).as_ref(),
    ...
    let signature = hex::encode(hmac_sha256(derived_sk.as_ref(), string_to_sign.as_bytes()).as_ref());
...
const RFC3986_RESERVED_CHARACTERS: &percent_encoding::AsciiSet = &percent_encoding::NON_ALPHANUMERIC
    .remove(b'_')
    .remove(b'-')
    .remove(b'~')
    .remove(b'.');

fn percent_encode_rfc3986(data: &[u8]) -> percent_encoding::PercentEncode<'_> {
...
fn get_hashed_request(req: &Request, headers: &str, signed_headers: &str, body: &str) -> String {
    ...
    // should sort first and then percent encode key/value.
    query.sort_unstable();
    let query = query
        .into_iter()
        .map(|(key, value)| {
            format!(
                "{key}={value}",
                key = percent_encode_rfc3986(key.as_bytes()),
                value = percent_encode_rfc3986(value.as_bytes()),
            )
        })
        .collect::<Vec<_>>()
        .join("&");
...
fn get_headers(headers: &HeaderMap, signed_headers: Option<&[HeaderName]>) -> anyhow::Result<(String, String)> {
...
mod tests {
    ...
    fn test_sign() {
        ...
        let req = Request::new(Method::POST, Url::parse("https://cvm.tencentcloudapi.com").unwrap());
